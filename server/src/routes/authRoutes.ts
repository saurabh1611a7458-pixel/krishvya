...
function generateToken(user: { id: string; phone: string; role: string; name: string }) {
  return jwt.sign(
    {
      id: user.id,
      phone: user.phone,
...
authRoutes.post('/signup', async (req, res: Response) => {
  try {
    const {
      name,
      phone,
...
authRoutes.post('/verify-otp', async (req, res: Response) => {
  try {
...
authRoutes.get('/me', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
